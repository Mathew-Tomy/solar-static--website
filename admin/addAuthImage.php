<?php
$destpath = '../uploads/large/banner'.date(dmY).date(His);
...
$thumb1Temp='../uploads/thumb/gal'.date(dmY).date(His) .$_FILES["file"]["name"][$key];   // Path where thumb nail image will be stored
...
// upload Logo
while(list($key,$value) = each($_FILES["logo"]["name"]))
	{
...
if (($_FILES["logo"]["type"][$key] == "image/gif")

|| ($_FILES["logo"]["type"][$key] == "image/jpeg")

|| ($_FILES["logo"]["type"][$key] == "image/pjpeg")

|| ($_FILES["logo"]["type"][$key] == "image/png")

&& ($_FILES["logo"]["size"][$key] < 2000000))

    {

$source = $_FILES["logo"]["tmp_name"][$key] ;
...
$thumb2Temp='../uploads/thumb/logo'.date(dmY).date(His) .$_FILES["logo"]["name"][$key];   // Path where thumb nail image will be stored
...
$im=ImageCreateFromGIF($image2Temp);
...
if($_FILES["logo"]["type"][$key]=="image/png"){
