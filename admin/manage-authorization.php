...
define('TABLE','tbl_authorization');
...
$db->query("delete from ". TABLE ." where auth_id in(".$_REQUEST['deleterec'].")");
redirect('manage-authorization.php','dtd=1&recordcount='.$arrLen.'');
...
//$searchText=trim($searchText);
		//total  count 
		  $sqlskr=$db->query("select * from ".TABLE." order by auth_id desc");
    $no_rows= $sqlskr->num_rows;
			
		   //$no_of_rows = $db->get_count(TABLE );
		
		//if(!$sort) $sort ='auth_id'; // sort by  
											...
											<table id="dynamic-table" class="table table-striped table-bordered table-hover tbl_repeat" >
												...
												while($rows=$sqlskr->fetch_assoc())
												{
													...
													<tr >
														<td class="center">
															<label class="pos-rel">
																<input type="checkbox" class="ace" name="del" value="<?=$rows['auth_id']?>"/>
																<span class="lbl"></span>
															</label>
														</td>
													...
													<td><?php if($rows['auth_logo']!=""){?>
														  <a href="#" class="catimg"  style="outline:none;marign-right:20px;">View
                <span><img src="../uploads/large/<?=$rows['auth_logo'];?>" style="outline:none" height="50px" width="50px"  /></span></a>
													<? }?></td>
														 
														<td>
															<?=shorten($rows['auth_desc']);?>															
														</td> 
 
														<td>

														
															<a class="green" href="edit-authorization.php?aid=<?=$rows['auth_id'];?>">
														...
														<td>
															<div class="hidden-sm hidden-xs action-buttons">
																...
																<a href="#" onClick='dele("<?php echo "delete-authorization.php?auth_id=$auth_id";?>")' class="tooltip-error" data-rel="tooltip" title="Delete">
																				<span class="red">
																					<i class="ace-icon fa fa-trash-o bigger-120"></i>
																				</span>
																			</a>
